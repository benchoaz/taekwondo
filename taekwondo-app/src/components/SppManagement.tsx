...
export default function SppManagement() {
  ...
  // Individual billing states (Pelatih Terbitkan Tagihan Perorangan)
  const [selectedIndivMember, setSelectedIndivMember] = useState("");
  const [indivPurpose, setIndivPurpose] = useState("");
  const [indivAmount, setIndivAmount] = useState("");
  const [indivDueDate, setIndivDueDate] = useState("");
  const [isSubmittingIndiv, setIsSubmittingIndiv] = useState(false);

  // Coach Cash Settlement states (Laporan Setoran Uang Tunai Pelatih)
  const [coachCashData, setCoachCashData] = useState<any[]>([]);
  const [totalCoachCash, setTotalCoachCash] = useState(0);
  const [isLoadingCoachCash, setIsLoadingCoachCash] = useState(false);

  const fetchCoachCash = async () => {
    setIsLoadingCoachCash(true);
    try {
      const res = await fetch("/api/payments/coach-cash");
      if (res.ok) {
        const data = await res.json();
        setCoachCashData(data.data || []);
        setTotalCoachCash(data.totalCashCollected || 0);
      }
    } catch (e) {
      console.error(e);
    } finally {
      setIsLoadingCoachCash(false);
    }
  };

  const handleIssueIndividualBilling = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedIndivMember || !indivPurpose || !indivAmount) {
      alert("Silakan lengkapi anggota, peruntukan tagihan, dan nominal.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/payments", {
        ...
        body: JSON.stringify({
          action: "individual-billing",
          memberId: selectedIndivMember,
          purpose: indivPurpose,
          amount: parseFloat(indivAmount),
          dueDate: indivDueDate || undefined,
        }),
      });
      const data = await res.json();
      if (res.ok) {
        alert("✅ Tagihan perorangan berhasil diterbitkan & notifikasi terkirim ke siswa!");
        setSelectedIndivMember("");
        setIndivPurpose("");
        setIndivAmount("");
        setIndivDueDate("");
        fetchInvoices();
      } else {
        alert(data.error || "Gagal menerbitkan tagihan perorangan.");
      }
    } catch (err) {
      console.error(err);
