  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center bg-[#0F172A] font-sans overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        ...
        {/* Login Box Panel */}
        <div className="bg-[#F8FAFC] rounded-[24px] px-8 py-8 shadow-2xl border border-slate-100 flex flex-col items-center">
          
          {/* Logo icon header */}
          <div className="flex flex-col items-center justify-center gap-3 mb-6">
            <img src={settings.logoUrl || "/logo.png"} alt="Logo" className="w-14 h-14 object-contain" />
            <div className="flex flex-col items-center text-center">
              <h2 className="text-xl font-black text-[#0F172A] tracking-tight leading-tight">{settings.dojangName}</h2>
              <span className="text-gray-400 text-[10px] font-bold uppercase tracking-wider mt-1">{settings.motto}</span>
            </div>
          </div>

          {errorMsg && (
            <div className="w-full mt-6 bg-red-50 border border-red-200 text-red-700 p-3 rounded-lg text-center text-xs font-bold">
              {errorMsg}
            </div>
          )}
          ...
          <form onSubmit={handleSecureLogin} className="w-full flex flex-col gap-4 mt-6">
            ...
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  placeholder="••••••••" 
                  required
                  className="w-full bg-white border border-[#0F172A]/10 rounded-xl pl-10 pr-10 py-3 text-xs text-[#0F172A] font-medium outline-none focus:ring-2 focus:ring-[#E10600] focus:border-transparent transition-all"
                />
                <Lock className="w-4 h-4 text-gray-400 absolute left-3.5 top-4" />
                <button 
                  type="button" 
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3.5 top-4 text-gray-400 hover:text-[#0F172A]"
                >
            ...
            <div className="flex items-center gap-2 mt-1">
              <input type="checkbox" id="remember" className="rounded text-[#E10600] focus:ring-[#E10600] border-gray-300 w-4 h-4" />
              <label htmlFor="remember" className="text-xs text-gray-400 font-medium cursor-pointer selection:bg-transparent">Remember me for 30 days</label>
            </div>

            <button 
              type="submit" 
              disabled={isLoading}
              className="w-full bg-[#E10600] hover:bg-[#C00500] text-white py-3 rounded-xl font-bold text-xs shadow-md shadow-[#E10600]/25 transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer mt-2 disabled:opacity-50"
            >
          ...
          <button 
            ...
            className="w-full bg-white border border-slate-200 text-gray-700 py-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-slate-50 transition-all active:scale-95 cursor-pointer mb-3"
          >
          ...
          <button 
            ...
            className="w-full bg-[#0F172A] text-white py-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-slate-800 transition-all active:scale-95 cursor-pointer"
          >
            <Mail className="w-4 h-4" />
            Send Magic Link to Email
          </button>

          <span className="text-[10px] text-gray-400 font-semibold block text-center mt-6">
