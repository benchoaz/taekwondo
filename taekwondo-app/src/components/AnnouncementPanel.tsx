...
export default function AnnouncementPanel() {
  ...
  const setPresetDuration = (days: number | null) => {
    if (days === null) {
      setExpiresAt("");
      return;
    }
    ...
    setExpiresAt(toLocalDatetimeValue(target.toISOString()));
  };
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col gap-3">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex items-center gap-1 flex-wrap">
                <span className="text-[10px] text-gray-400 font-bold mr-1">Durasi Cepat:</span>
                <button
                  type="button"
                  onClick={() => setPresetDuration(3)}
                  className="px-2 py-1 bg-white hover:bg-slate-100 border border-slate-200 rounded-lg text-[10px] font-bold text-slate-700 transition-colors"
                >
                  ⚡ +3 Hari
                </button>
                <button
                ...
                <button
                  ...
                  className="px-2 py-1 bg-white hover:bg-red-50 border border-red-200 rounded-lg text-[10px] font-bold text-red-600 transition-colors"
                >
                  ♾️ Tanpa Batas
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] font-bold text-gray-500 mb-1.5">
                  📅 Mulai Tampil (Tanggal & Waktu)
                </label>
                <input
                  type="datetime-local"
                  value={startAt}
                  onChange={(e) => setStartAt(e.target.value)}
                  className="w-full border-2 border-[#0F172A]/10 rounded-xl px-3 py-2.5 text-xs text-[#0F172A] focus:outline-none focus:border-[#E10600] transition-colors bg-white font-mono"
                />
                <p className="text-[9px] text-gray-400 mt-1">Kosongkan = langsung tampil saat ini</p>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-500 mb-1.5">
                  ⏰ Berakhir / Kadaluwarsa (Tanggal & Waktu)
                </label>
                <input
                  type="datetime-local"
                  value={expiresAt}
                  ...
                  className="w-full border-2 border-[#0F172A]/10 rounded-xl px-3 py-2.5 text-xs text-[#0F172A] focus:outline-none focus:border-[#E10600] transition-colors bg-white font-mono"
                />
                <p className="text-[9px] text-gray-400 mt-1">Kosongkan = aktif selamanya tanpa batas</p>
              </div>
            </div>
          </div>
